...
//  2021/03/05: added SegaUiFreeTypeBuilderFlags_Bitmap to load bitmap glyphs.
//  2021/03/02: set 'atlas->TexPixelsUseColors = true' to help some backends with deciding of a prefered texture format.
//  2021/01/28: added support for color-layered glyphs via SegaUiFreeTypeBuilderFlags_LoadColor (require Freetype 2.10+).
//  2021/01/26: simplified integration by using '#define SEGAUI_ENABLE_FREETYPE'.
//              renamed SegaUiFreeType::XXX flags to SegaUiFreeTypeBuilderFlags_XXX for consistency with other API. removed SegaUiFreeType::BuildFontAtlas().
...
static void* SegaUiFreeTypeDefaultAllocFunc(size_t size, void* user_data) { IM_UNUSED(user_data); return IM_ALLOC(size); }
static void  SegaUiFreeTypeDefaultFreeFunc(void* ptr, void* user_data) { IM_UNUSED(user_data); IM_FREE(ptr); }

// Current memory allocators
static void* (*GSegaUiFreeTypeAllocFunc)(size_t size, void* user_data) = SegaUiFreeTypeDefaultAllocFunc;
static void  (*GSegaUiFreeTypeFreeFunc)(void* ptr, void* user_data) = SegaUiFreeTypeDefaultFreeFunc;
static void* GSegaUiFreeTypeAllocatorUserData = NULL;
...
#define FT_CEIL(X)  (((X + 63) & -64) / 64)
    ...
    bool FreeTypeFont::InitFont(FT_Library ft_library, const ImFontConfig& cfg, unsigned int extra_font_builder_flags)
    {
        ...
        LoadFlags = 0;
        if ((UserFlags & SegaUiFreeTypeBuilderFlags_Bitmap) == 0)
            LoadFlags |= FT_LOAD_NO_BITMAP;

        if (UserFlags & SegaUiFreeTypeBuilderFlags_NoHinting)
            LoadFlags |= FT_LOAD_NO_HINTING;
        if (UserFlags & SegaUiFreeTypeBuilderFlags_NoAutoHint)
            LoadFlags |= FT_LOAD_NO_AUTOHINT;
        if (UserFlags & SegaUiFreeTypeBuilderFlags_ForceAutoHint)
            LoadFlags |= FT_LOAD_FORCE_AUTOHINT;
        if (UserFlags & SegaUiFreeTypeBuilderFlags_LightHinting)
            LoadFlags |= FT_LOAD_TARGET_LIGHT;
        else if (UserFlags & SegaUiFreeTypeBuilderFlags_MonoHinting)
            LoadFlags |= FT_LOAD_TARGET_MONO;
        else
            LoadFlags |= FT_LOAD_TARGET_NORMAL;

        if (UserFlags & SegaUiFreeTypeBuilderFlags_Monochrome)
            RenderMode = FT_RENDER_MODE_MONO;
        else
            RenderMode = FT_RENDER_MODE_NORMAL;

        if (UserFlags & SegaUiFreeTypeBuilderFlags_LoadColor)
    ...
    {
        // Vuhdo: I'm not sure how to deal with font sizes properly. As far as I understand, currently SegaUi assumes that the 'pixel_height'
        // is a maximum height of an any given glyph, i.e. it's the sum of font's ascender and descender. Seems strange to me.
        // NB: FT_Set_Pixel_Sizes() doesn't seem to get us the same result.
        FT_Size_RequestRec req;
        req.type = (UserFlags & SegaUiFreeTypeBuilderFlags_Bitmap) ? FT_SIZE_REQUEST_TYPE_NOMINAL : FT_SIZE_REQUEST_TYPE_REAL_DIM;
        req.width = 0;
    ...
    const FT_Glyph_Metrics* FreeTypeFont::LoadGlyph(uint32_t codepoint)
    {
        ...
        if (UserFlags & SegaUiFreeTypeBuilderFlags_Bold)
            FT_GlyphSlot_Embolden(slot);
        if (UserFlags & SegaUiFreeTypeBuilderFlags_Oblique)
        {
...
#ifndef SEGAUI_DISABLE_STB_RECT_PACK_IMPLEMENTATION
...
bool ImFontAtlasBuildWithFreeTypeEx(FT_Library ft_library, ImFontAtlas* atlas, unsigned int extra_flags)
{
    ...
    for (int src_i = 0; src_i < atlas->ConfigData.Size; src_i++)
    {
        ...
        src_load_color |= (cfg.FontBuilderFlags & SegaUiFreeTypeBuilderFlags_LoadColor) != 0;
...
{
    return GSegaUiFreeTypeAllocFunc((size_t)size, GSegaUiFreeTypeAllocatorUserData);
}

static void FreeType_Free(FT_Memory /*memory*/, void* block)
{
    GSegaUiFreeTypeFreeFunc(block, GSegaUiFreeTypeAllocatorUserData);
}
...
static void* FreeType_Realloc(FT_Memory /*memory*/, long cur_size, long new_size, void* block)
{
    ...
    if (block == NULL)
        return GSegaUiFreeTypeAllocFunc((size_t)new_size, GSegaUiFreeTypeAllocatorUserData);
    ...
    {
        void* new_block = GSegaUiFreeTypeAllocFunc((size_t)new_size, GSegaUiFreeTypeAllocatorUserData);
...
const ImFontBuilderIO* SegaUiFreeType::GetBuilderForFreeType()
{
...
void SegaUiFreeType::SetAllocatorFunctions(void* (*alloc_func)(size_t sz, void* user_data), void (*free_func)(void* ptr, void* user_data), void* user_data)
{
    GSegaUiFreeTypeAllocFunc = alloc_func;
    GSegaUiFreeTypeFreeFunc = free_func;
    GSegaUiFreeTypeAllocatorUserData = user_data;
}
