...
uint64_t intel_driver::MmAllocateContiguousMemory(HANDLE device_handle, SIZE_T NumberOfBytes, LARGE_INTEGER HighestAcceptableAddress) {
	...
	static uint64_t kernel_MmAllocateContiguousMemory = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, "MmAllocateContiguousMemory"); //FsRtlAllocatePoolWithTag
	...
	uint64_t allocated_pool = 0;//0x11 //PagedPool 0x610

	if (!CallKernelFunction(device_handle, &allocated_pool, kernel_MmAllocateContiguousMemory, NumberOfBytes, HighestAcceptableAddress)) //Changed pool tag since an extremely meme checking diff between allocation size and average for detection....
		return 0;
...
HANDLE intel_driver::Load() {
	...
	//from https://github.com/ShoaShekelbergstein/kmdf_loader2 as some Drivers takes same device name
	if (intel_driver::IsRunning()) {
		Log(L"[-] \\Device\\Nal is already in use. unloading it.." << std::endl);
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size) {
	...
	static uint64_t kernel_ExAllocatePool = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, "FsRtlAllocatePoolWithTag"); //FsRtlAllocatePoolWithTag
	...
	if (!call_kernel_function(device_handle, &allocated_pool, kernel_ExAllocatePool, pool_type, size, 0x6D566343)) //Changed pool tag since an extremely meme checking diff between allocation size and average for detection....
		return 0;
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size) {
	...
	static uint64_t kernel_ExAllocatePool = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, "FsRtlAllocatePoolWithTag");

	if (!kernel_ExAllocatePool) {
		Log(L"[!] Failed to find FsRtlAllocatePoolWithTag" << std::endl);
		return 0;
	}

	uint64_t allocated_pool = 0;//0x11 //PagedPool 0x610

	if (!CallKernelFunction(device_handle, &allocated_pool, kernel_ExAllocatePool, pool_type, size, 0x6D566343))
		return 0;
